...
import {
  ...
  Upload, Popconfirm, message, type TableColumnsType, type UploadFile,
} from "antd";
...
export default function Documentos() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      messageApi.error("Error al subir documento");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const columns: TableColumnsType<Documento> = [
    ...
    },
    {
      title: "Tarea",
      dataIndex: "id_tarea",
      render: (id) => tareas.find((t) => t.id_tarea === id)?.nombre ?? "-",
    },
    {
      title: "Acciones",
      width: 120,
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="id_tarea" label="Tarea" rules={[{ required: true }]}>
            <Select placeholder="Selecciona tarea">
              {tareas.map((t) => (
